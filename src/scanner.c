...
void *tree_sitter_mlua_external_scanner_create()
{
  // calloc, not malloc: state->started must start at 0 (the "not inside a
  // comment/string" sentinel the switch in scan() below relies on) --
  // deserialize() only ever writes into it when a prior serialized state
  // exists (length == 2), so the very first scan() call after create()
  // (nothing to deserialize yet) would otherwise read whatever garbage
  // malloc happened to leave behind. When that garbage byte collided with
  // one of the enum StartedToken values, scan() took a wrong branch on the
  // first call and misparsed the rest of the file -- confirmed directly: a
  // file with no comment/string before its first statement (nothing to
  // "prime" state->started back to 0 via a real token) reliably produced
  // spurious STRING/comment nodes, while the same file with a leading
  // comment parsed cleanly. Upstream tree-sitter-lua's own scanner.c uses
  // ts_calloc here for exactly this reason.
  return calloc(1, sizeof(struct ScannerState));
}

void tree_sitter_mlua_external_scanner_destroy(void *payload)
{
  free(payload);
}

unsigned int tree_sitter_mlua_external_scanner_serialize(void *payload, char *buffer)
{
...
void tree_sitter_mlua_external_scanner_deserialize(void *payload, const char *buffer, unsigned int length)
{
  if (length == 2)
  {
    struct ScannerState *state = payload;
...
bool tree_sitter_mlua_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols)
{
  struct ScannerState *state = payload;
